# services/auth-service/app/extensions/extensions.py
"""
...
socketio = SocketIO(
    cors_allowed_origins=[
        ...
        "http://196.253.26.113",
        "*"  # temporary broad fallback – tighten in production
    ],
    logger=True,
...
def init_redis(app: Flask) -> None:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # Support both modern bool and old Redis bytes/string
...
# ────────────────────────────────────────────────
# Initialize all extensions (call once in create_app)
# ────────────────────────────────────────────────
...
def init_extensions(app: Flask) -> None:
    ...
    # CORS – explicit coverage for Socket.IO
    cors.init_app(
        app,
        resources={
            r"/api/*": {"origins": "*"},
...
# ────────────────────────────────────────────────
# Redis pub/sub listener for live logs
# ────────────────────────────────────────────────
